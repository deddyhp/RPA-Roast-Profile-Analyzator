...
def parse_alog(file_name: str, payload: bytes) -> ParsedRoast:
    ...
    events = pd.DataFrame(
        event_rows,
        columns=["event_name", "time_s", "bt", "et", "value", "source"],
    )
    ...
    with db() as con:
        ...
        con.execute(
            ...
            INSERT INTO artisan_files(
                roast_session_id, original_name, stored_path, checksum_sha256,
        ...
        con.executemany(
            ...
            INSERT INTO roast_curve_points(
                roast_session_id, time_s, bt, et, ror_bt
            ) VALUES (?, ?, ?, ?, ?)
            """,
            [
                (
        ...
        if not parsed.events.empty:
            con.executemany(
                ...
                INSERT INTO roast_events(
                    roast_session_id, event_name, time_s, bt, et, value, source
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                [
                    (
...
def render_roast_detail(roast_id: str) -> None:
    with db() as con:
        ...
        events = pd.read_sql_query(
            """
            SELECT event_name, time_s, bt, et, value, source
            FROM roast_events
            WHERE roast_session_id = ?
            ORDER BY time_s
            """,
...
def main() -> None:
    ...
    st.title("🔥 RPA")
    st.caption(f"Roast Profile Analyzator · Database Foundation V{APP_VERSION}")

    page = st.sidebar.radio(
        "Menu",
        ["Home", "Import Roast", "Roast Database", "Release History"],
    )
